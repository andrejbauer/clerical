...
let float =
  [%sedlex.regexp?
    Opt '-', Opt ("0x" | "0X" | "0b" | "0B"), Plus hexdigit, '.', Star hexdigit]
...
let safe_int_of_string lexbuf =
  ...
  with Invalid_argument _ ->
    Ulexbuf.error ~loc:(loc_of lexbuf) (Ulexbuf.BadNumeral s)
...
and token_aux ({ Ulexbuf.stream; _ } as lexbuf) =
  ...
  | infixop1 ->
      f ();
      INFIXOP1 (Ulexbuf.lexeme lexbuf, loc_of lexbuf)
  | infixop2 ->
      f ();
      INFIXOP2 (Ulexbuf.lexeme lexbuf, loc_of lexbuf)
  (* Comes before infixop3 because ** matches the infixop3 pattern too *)
  | infixop4 ->
      f ();
      INFIXOP4 (Ulexbuf.lexeme lexbuf, loc_of lexbuf)
  | infixop3 ->
      f ();
      INFIXOP3 (Ulexbuf.lexeme lexbuf, loc_of lexbuf)
  | eof ->
      f ();
      EOF
  | name -> (
      ...
      try List.assoc n reserved with Not_found -> NAME n)
  | float ->
      f ();
      let r = Ulexbuf.lexeme lexbuf in
      FLOAT r
  | numeral ->
      f ();
      let k = safe_int_of_string lexbuf in
      NUMERAL k
  | any ->
      f ();
...
let run (lexer : Ulexbuf.t -> 'a)
    (parser : (Lexing.lexbuf -> 'a) -> Lexing.lexbuf -> 'b) (lexbuf : Ulexbuf.t)
    : 'b =
  let lexer () =
...
let read_toplevel parse () =
  ...
  let ends_with_backslash_or_empty str =
    ...
    if i >= 0 && str.[i] = '\\' then (true, String.sub str 0 i)
  ...
  let rec read_more prompt acc =
    ...
    let acc = acc ^ "\n" ^ str in
    if more then read_more "  " acc else acc
  in
