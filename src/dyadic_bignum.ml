(** Dyadic numbers with [Big_int] *)

(* Dyadic numbers with the Ocaml [Big_int] package. This is a lot slower than
...
let get_prec x =
  let a = Big_int.nat_of_big_int (Big_int.abs_big_int x) in
    let l = Big_int.num_digits_big_int x in
      l * Nat.length_of_digit - (Nat.num_leading_zero_bits_in_digit a (l-1))

let rounddir = function
  | Some Down | Some Near | None -> fun x->x
  | Some Up -> Big_int.minus_big_int
...
let round_dyadic ?prec ~round (m,e) =
  match prec with
    ...
    | Some p0 ->
      ...
      (if p1 <= p0 then Big_int.shift_left_big_int m (p0-p1)
      else
	match round with
	| Down -> Big_int.shift_right_big_int m (p1-p0)
	| Up -> Big_int.minus_big_int (Big_int.shift_right_big_int
                                         (Big_int.minus_big_int m) (p1-p0))
	| Near ->
           if 0 == Big_int.sign_big_int (Big_int.extract_big_int m 0 (p1-p0-1)) &&
              0 == Big_int.sign_big_int (Big_int.extract_big_int m (p1-p0+1) 1)
           then
	     Big_int.shift_right_big_int m (p1-p0)
	   else
	     Big_int.shift_right_big_int (Big_int.succ_big_int
                                            (Big_int.shift_right_big_int m (p1-p0-1))) 1
       ), e-(p0-p1)
...
let of_int ?prec ~round k = dyadic (round_dyadic ?prec ~round (Big_int.big_int_of_int k, 0))
...
let make_int ?prec ~round m e =
  make ?prec ~round (Big_int.big_int_of_int m) e
...
let common_base (m1, e1) (m2, e2) =
  let c = e1 - e2 in
    if c < 0 then (m1, Big_int.shift_left_big_int m2 (-c), e1)
    else if c > 0 then (Big_int.shift_left_big_int m1 c, m2, e2)
    else (m1, m2, e1)
  ...
  match a, b with
    ...
    | Dyadic (m1,e1), Dyadic (m2,e2) ->
       let (m1, m2, _) = common_base (m1,e1) (m2,e2) in  (*optimize*)
         let c = Big_int.compare_big_int m1 m2 in
            if c<0 then `less
...
let sgn = function
  ...
  | PosInf -> `positive
  | Dyadic (m,_) ->
     let s = Big_int.sign_big_int m in
...
let add ?prec ~round a b =
  match a, b with
	...
	dyadic (round_dyadic ?prec ~round (Big_int.add_big_int m1 m2, e))
...
let sub ?prec ~round a b =
  match a, b with
    ...
    | Dyadic (m1,e1), Dyadic (m2,e2) ->
      let (m1, m2, e) = common_base (m1,e1) (m2,e2) in
	dyadic (round_dyadic ?prec ~round (Big_int.sub_big_int m1 m2, e))
...
let neg ?prec ~round = function
  ...
  | PosInf -> NegInf
  | Dyadic (m,e) ->
      dyadic (round_dyadic ?prec ~round (Big_int.minus_big_int m, e))
...
let mul ?prec ~round a b =
  match a, b with
    ...
    | Dyadic (m1,e1), Dyadic (m2,e2) ->
	dyadic (round_dyadic ?prec ~round (Big_int.mult_big_int m1 m2, e1+e2))
    | Dyadic (m,e), PosInf
    | PosInf, Dyadic (m,e) ->
	(let s = Big_int.sign_big_int m in
	   if s < 0 then NegInf
...
let pow ?prec ~round a k =
  match a with
    ...
    | Dyadic (m,e) -> dyadic (round_dyadic ?prec ~round
                                (Big_int.power_big_int_positive_int m k, e*k))
    | PosInf -> PosInf
...
let div ~prec ~round a b =
  match a, b with
    ...
    | Dyadic (m1,e1), Dyadic (m2,e2) ->
	if Big_int.sign_big_int m2 = 0 then
	  infinity round
	else
	    let (m1,m2, _) = common_base (m1,e1) (m2,e2) in
	    let k = Pervasives.max (prec - (get_prec m1) + (get_prec m2)) 0 in
	    let m1 = Big_int.shift_left_big_int m1 k in
	    let (q,r) = Big_int.quomod_big_int m1 m2 in
	    let p1 = (match round with
		| Up -> Big_int.sign_big_int r * Big_int.sign_big_int m2
		| Down -> 0
		| Near ->
                   let c = Big_int.compare_big_int (Big_int.shift_left_big_int r 1)
                             (Big_int.abs_big_int m2) in
		   if c == 0 then
                     Big_int.int_of_big_int (Big_int.extract_big_int q 0 1) *
                       Big_int.sign_big_int q
                   else if c > 0 then
                     Big_int.sign_big_int m2
                   else 0
	    ) in
	      dyadic (Big_int.add_int_big_int p1 q, -k)
...
let inv ~prec ~round = function
  ...
  | Dyadic (m,e) ->
      if Big_int.sign_big_int m = 0 then
	infinity round
      else
...
let to_base10 (m, e) =
    ...
    let p2 = (pow ~round:near (dyadic (Big_int.big_int_of_int 5,1)) (abs e10)) in
...
let of_string ~round str =
   ...
   let (m,e) = (Big_int.big_int_of_string (m1^m2),e1+e) in
      dyadic (from_base10 ~round ~prec:53 (m,e))
...
let to_string2 = function
  ...
  | NegInf -> "-inf"
  | Dyadic (m,e) ->
      (Big_int.string_of_big_int m)^"p"^(string_of_int e)
...
let to_string x =
  let exp_notation = 4 in
    match x with
	  ...
	  let s = Big_int.string_of_big_int (Big_int.abs_big_int m) in
	  let e = e + String.length s in
	  let sign = if Big_int.sign_big_int m < 0 then "-" else "" in
