(** Dyadic numbers with MPFR, wrapped up in a purely functional interface.
    ...
    We always specify output precision [prec], which measures bits of mantissa,
    and rounding mode. In other words, all operations are approximate. We never
    need to add or multiply two dyadics exactly (it is too expensive to do so,
    anyway). *)
...
let min a b = if Mpfr.cmp a b < 0 then a else b
let max a b = if Mpfr.cmp a b < 0 then b else a
...
let sgn a =
  let c = Mpfr.sgn a in
  if c < 0 then `negative else if c = 0 then `zero else `positive
...
let is_positive_infinity a = is_infinity a && positive a
let is_negative_infinity a = is_infinity a && negative a
...
(** Arithmetical operations need to take care of infinite operands when the
    result would be [nan] (not a number). *)
...
let add ?prec ~round a b =
  ...
  in
  if
    (is_negative_infinity a && is_positive_infinity b)
...
let sub ?prec ~round a b =
  ...
  in
  if
    (is_negative_infinity a && is_negative_infinity b)
...
let mul ?prec ~round a b =
  let prec =
    match prec with None -> Mpfr.get_prec a + Mpfr.get_prec b | Some p -> p
  in
...
let inv ~prec ~round a =
  if is_zero a || is_infinity a then infinity round
  else
    let q = Mpfr.init2 prec in
...
(** [average a b] returns a dyadic which is guaranteed to be strictly between
    [a] and [b], close to the average. This only works for finite [a] and [b].
*)
let average a b =
...
(** [Mpfr.to_string] produces rather unreadable results. This is an improved
    version. *)
let to_string ~round x =
  ...
  | `number ->
      ...
      else if e > 0 then sign ^ string_insert str e "."
      else sign ^ "0." ^ String.make (-e) '0' ^ str

let get_exp x = Mpfr.get_exp x
