...
(** A signature for dyadic rationals. These are the rationals
...
sig
  ...
  val to_string : round:rounding_mode -> t -> string
  val get_exp : t -> int
end;;
...
struct
  ...
  let to_string i =
    "[" ^ D.to_string ~round:D.down (lower i) ^ "," ^ D.to_string ~round:D.up (upper i) ^ "]"
