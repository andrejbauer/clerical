...
let exec_interactive ~pool { desugar; typecheck; runtime } =
  ...
  let runtime = Eval.toplevel ~pool ~quiet:false runtime cmd in
  { desugar; typecheck; runtime }

let load_file ~pool ~quiet { desugar; typecheck; runtime } fn =
  let desugar, cmds = Desugar.load desugar fn in
  let typecheck, cmds = Typecheck.topfile typecheck cmds in
  let runtime = Eval.topfile ~pool ~quiet runtime cmds in
  { desugar; typecheck; runtime }
